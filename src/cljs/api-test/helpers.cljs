...
(defn user-typing-async []
  (go
    (let [payload (<! typing-chan)]
      ...
      (loop []
        (let [[v c] (alts! [just-send-a-message-chan typing-chan (timeout 1300)])] ; should probably have a third chan here, to represent someone posting a message. That should end "user is typing" straight away.
            (cond
